...
wxString ConfigFile::GetChatDefPath() const
{
    ...
    // 4. Look for <DefBin> node and get its Path attribute
    for (wxXmlNode* defBin = paramsContainerNode->GetChildren(); defBin; defBin = defBin->GetNext())
    {
        if (defBin->GetType() != wxXML_ELEMENT_NODE) continue;
        if (defBin->GetName().CmpNoCase("DefBin") != 0) continue;

        wxString path = defBin->GetAttribute("Path", "");
        if (path.IsEmpty()) path = defBin->GetAttribute("path", "");
        if (!path.IsEmpty())
        {
            res = path;
            break;
